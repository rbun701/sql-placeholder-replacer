...
def align_clause_block(clause: str, expressions: list, indent_level=2) -> str:
    parsed = []
    max_len = 0
...
def format_clause_block(sql: str, clause: str) -> str:
    pattern = re.compile(rf'(\b{clause}\b)(.*?)(?=\bFROM\b|\bWHERE\b|\bHAVING\b|\bGROUP BY\b|\bORDER BY\b|\bLIMIT\b|$)', re.IGNORECASE | re.DOTALL)
    ...
    for match in matches:
        ...
        sql = sql.replace(full, aligned_block)
    return sql
